  ...
  return (
    <div className="mx-auto flex max-w-[900px] flex-col items-center gap-8 px-14 py-20 text-center">
      ...
      <div className="flex items-center gap-3 text-[13px] font-medium">
        <span className="inline-flex items-center gap-2 rounded-full border border-[#BFE4DA] bg-verified-soft px-3.5 py-1.5 text-verified">Plan ready</span>
        <span className="h-px w-6 bg-[#D9D3C9]" />
        <span className="inline-flex items-center gap-2 rounded-full border border-[#D2D1F2] bg-accent-soft px-3.5 py-1.5 text-accent-ink">
          <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-[#C7C6EE] border-t-accent" />
          Researching
        </span>
        <span className="h-px w-6 bg-[#D9D3C9]" />
      ...
      <div className="w-full max-w-[440px] rounded-2xl border border-line bg-card p-4 text-left">
        {angles.map((angle, i) => (
          <div key={angle} className={`flex items-center gap-3 rounded-lg px-3 py-2.5 ${i === active ? "bg-[#F4F4FE]" : ""}`}>
            {i < active ? (
              <span className="flex h-6 w-6 items-center justify-center rounded-full bg-verified-soft">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#0B6B57" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6 9 17l-5-5"/></svg>
              </span>
            ) : i === active ? (
              <span className="h-6 w-6 animate-spin rounded-full border-2 border-[#C7C6EE] border-t-accent" />
            ) : (
              <span className="h-6 w-6 rounded-full border border-dashed border-[#D9D3C9]" />
            )}
